...
export function showInfoPanel(
  ...
  const bookCta = document.getElementById("book-cta");
  // Clear existing content (old book promo)
  bookCta.innerHTML = '';
  if (node.docs_links && Array.isArray(node.docs_links) && node.docs_links.length > 0) {
    // Build documentation links section
    const section = document.createElement('div');
    section.className = 'docs-links-section';
    const heading = document.createElement('h3');
    heading.textContent = 'Documentation & Resources';
    ...
    list.className = 'docs-links-list';
    list.style.listStyle = 'none';
    list.style.padding = '0';
    list.style.margin = '0';
    for (const link of node.docs_links) {
...
function setBookCtaVisible(isVisible) {
  ...
  bookCta.style.display = isVisible ? 'block' : 'none';
}
